...
import type { AnalysisTheme } from '@sentix/types';
...
export function TraceDrawer({ theme, onClose }: TraceDrawerProps) {
  const { analysis_theme, theme: aiTheme, signals } = theme as any;
    ...
    <Sheet open onOpenChange={() => onClose()} modal={false}>
      <SheetContent side="right" className="w-full md:max-w-2xl overflow-y-auto">
        <SheetHeader className="mb-6">
          <SheetTitle className="text-xl">{aiTheme.title}</SheetTitle>
          <SheetDescription>
            ...
            <div className="grid grid-cols-2 gap-3">
              ...
              <div className="rounded-lg bg-muted p-3">
                ...
                <p className="text-lg font-bold text-destructive">
                  {formatCurrency(analysis_theme?.revenue_lost || 0)}
                </p>
                {analysis_theme?.revenue_at_risk > 0 && (
                  <p className="text-xs text-warning">
                    +{formatCurrency(analysis_theme.revenue_at_risk)} at risk
                  </p>
                )}
              </div>
              ...
              <div className="rounded-lg bg-muted p-3">
                ...
                <Badge variant="secondary" className={`mt-1 ${getEffortBucketColor(analysis_theme?.effort_bucket || '')}`}>
              ...
              <div className="flex items-center gap-4">
                <div className="flex-1 h-2 rounded-full bg-secondary">
                  <div
                    ...
                    style={{ width: `${(analysis_theme?.confidence || 0) * 100}%` }}
                  />
                </div>
