...
describe('OpenRouterClient', () => {
    ...
    it('should throw error if OPENROUTER_API_KEY is missing', () => {
        ...
        expect(() => new openRouter_1.OpenRouterClient()).toThrow('OPENROUTER_API_KEY environment variable is required');
    });
    it('successful API call returns parsed themes', async () => {
        ...
        const client = new openRouter_1.OpenRouterClient();
        const context = {
    ...
    it('500 response throws error with status', async () => {
        ...
        await expect(client.extractThemes(context)).rejects.toThrow('OpenRouter error 500: Internal server error');
    });
    ...
    it('aborts fetch after 30 second timeout and retries', async () => {
        ...
        const mockFetch = jest.fn((input, options) => {
            fetchAttempt++;
            if (fetchAttempt === 1) {
...
//# sourceMappingURL=openRouterClient.test.js.map