...
async function registerRoutes(server) {
    ...
    server.get('/analysis/:projectId', { preValidation: [server.authenticate] }, async (request, reply) => {
    ...
    server.get('/analysis/history/:projectId', { preValidation: [server.authenticate] }, async (request, reply) => {
        const user = request.user;
        const { projectId } = request.params;
        // Verify project belongs to user's org
        const project = await request.prisma.project.findUnique({
    ...
    server.get('/analysis/:id', { preValidation: [server.authenticate] }, async (request, reply) => {
        const user = request.user;
    ...
    server.delete('/analysis/:id', { preValidation: [server.authenticate] }, async (request, reply) => {
        const user = request.user;
        const { id } = request.params;
        const analysis = await request.prisma.analysis.findUnique({
            where: { id },
    ...
    server.get('/trace/:analysisThemeId', { preValidation: [server.authenticate] }, async (request, reply) => {
        const user = request.user;
        const { analysisThemeId } = request.params;
        ...
        const signalIds = analysisTheme.evidence_ids || [];
