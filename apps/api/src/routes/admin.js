...
async function registerRoutes(server) {
    ...
    server.get('/admin/queues', { preValidation: [server.authenticate] }, async (request, reply) => {
        ...
        const stats = await Promise.all(queues.map(async (queueName) => {
            const queue = global[`queue_${queueName}`];
            if (!queue)
                return { name: queueName, error: 'Queue not found' };
    ...
    server.post('/admin/queues/:queueName/retry-failed', { preValidation: [server.authenticate] }, async (request, reply) => {
        const user = request.user;
    ...
    server.get('/admin/health', { preValidation: [server.authenticate] }, async (request) => {
        const user = request.user;
        if (user.role !== 'ADMIN') {
            throw reply.code(403).send({ error: 'Forbidden', message: 'Admin access required' });
        }
        ...
        const workers = global.workerCount || 0;
        health.workers = { count: workers, status: workers > 0 ? 'healthy' : 'warning' };
        return reply.send(health);
    });
    // GET /admin/stats - System statistics
    server.get('/admin/stats', { preValidation: [server.authenticate] }, async (request, reply) => {
        const user = request.user;
        if (user.role !== 'ADMIN') {
            throw reply.code(403).send({ error: 'Forbidden', message: 'Admin access required' });
        }
        const [userCount, orgCount, projectCount, signalCount, analysisCount,] = await Promise.all([
